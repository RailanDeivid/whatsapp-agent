...
Assistente inteligente integrado ao WhatsApp com arquitetura **multi-agente**: um **Router LLM** classifica cada pergunta e roteia para o **Agente SQL** (Grok 4.1 Fast via OpenRouter + Dremio/MySQL, para dados de vendas e compras), o **Agente RAG** (Grok 4.1 Fast via OpenRouter + Chroma, para documentos internos), ou responde diretamente via **LLM** para saudações e perguntas gerais. Respostas gerais, mensagens de espera e visão de imagens usam o **GPT-4o mini** (modelo fallback econômico e estável). Áudios são transcritos via **OpenAI Whisper** antes de chegarem aos agentes. **Imagens** são interpretadas via um modelo com visão (configurável via `VISION_MODEL_NAME`) — texto, listas e perguntas contidas na imagem são extraídos e processados normalmente.
...
### Interação por Imagem
> Envie uma foto com texto, lista de nomes ou perguntas escritas — o agente lê a imagem e responde normalmente.
...
│           (chains.py também expõe extract_text_from_image — visão via VISION_MODEL_NAME)
...
```
texto/audio/imagem → pre-processamento → route_and_invoke()
                │
...
**Fallback de modelo:**
- **Respostas gerais e mensagens de espera:** sempre usam `FALLBACK_MODEL_NAME` (GPT-4o mini) — mais rápido e ~5x mais barato que o modelo principal para tarefas simples.
...
|---|---|---|---|
| `consultar_transacoes` | SQL | Dremio | Faturamento, receita, desempenho de vendas, ticket médio, fluxo |
...
# Troque aqui conforme novos modelos forem lançados no OpenRouter
FALLBACK_MODEL_NAME=openai/gpt-4o-mini

# Visão — modelo usado para interpretar imagens recebidas (padrão: mesmo que ROUTER_MODEL_NAME)
# Qualquer modelo com visão no OpenRouter funciona: openai/gpt-4o, google/gemini-flash-1.5, etc.
...
| **Geral** (`general_prompt`) | GPT-4o mini, sem ReAct. Saudações, perguntas conceituais, fora do escopo. Mais rápido e barato. |
...
| Variável | Modelo atual | Uso | Preço (input/output por 1M tokens) |
|---|---|---|---|
| `ROUTER_MODEL_NAME` | `x-ai/grok-4.1-fast` | Principal — SQL Agent, RAG Agent, Router | $0,20 / $0,50 |
| `FALLBACK_MODEL_NAME` | `openai/gpt-4o-mini` | Fallback — respostas gerais, espera, erros | $0,15 / $0,60 |
| `VISION_MODEL_NAME` | `openai/gpt-4o-mini` | Interpretação de imagens recebidas via WhatsApp | $0,15 / $0,60 |

**Outros modelos testados e compatíveis:**

| Modelo | OpenRouter ID | Indicado para |
|---|---|---|
| GPT-4o | `openai/gpt-4o` | Agente SQL com queries complexas |
| GPT-4.1 | `openai/gpt-4.1` | Agente SQL — melhor custo/benefício OpenAI |
| Claude Sonnet 4.5 | `anthropic/claude-sonnet-4-5` | Melhor desempenho em tool use e ReAct |
| Llama 3.3 70B | `meta-llama/llama-3.3-70b-instruct` | Alternativa open-source robusta |
| DeepSeek V3 | `deepseek/deepseek-chat` | Opção econômica com bom raciocínio |
| Gemini 2.0 Flash | `google/gemini-2.0-flash-001` | Rápido, mas sujeito a rate limit 429 |

> Para trocar de modelo basta alterar `ROUTER_MODEL_NAME`, `FALLBACK_MODEL_NAME` ou `VISION_MODEL_NAME` no `.env` e reiniciar o container — sem alterar código. Cache de prompt ativo via header `X-OpenRouter-Cache` — reduz custo do prompt do sistema ~75% em chamadas repetidas. Consulte [openrouter.ai/models](https://openrouter.ai/models) para preços e novos modelos disponíveis.
...
| `VISION_MODEL_NAME` (ex: `openai/gpt-4o-mini`) | Interpretação de imagens | ~$0,0001–0,0005/imagem |
...
| Mensagem geral / conceitual | GPT-4o mini (fallback) | ~$0,0003 | ~R$0,002 |
| Thinking message (espera) | GPT-4o mini (fallback) | ~$0,00005 | ~R$0,0003 |
| Excel (dados já em cache) | Zero LLM | $0,00 | R$0,00 |
| Áudio 30s + agente | Whisper + Grok | ~$0,004 | ~R$0,024 |
| Imagem + agente | Vision LLM + Grok | ~$0,002 | ~R$0,012 |
